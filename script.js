...
function shareholderReturns(exitValue) {
  ...
  let remainder =
    exitValue -
    prefAInvestorsLiqPref -
    prefBInvestorsLiqPref -
    prefCInvestorsLiqPref;
  ...
  if (prefCInvestorsTotalReturn >= prefCInvestorsLiqPref * 2) {
    // Has Series C reached cap? If so, so have B and A.
    ...
    if (prefCInvestorsTotalReturnCommon >= prefCInvestorsTotalReturn) {
      // Would C earn more converting to common shares than taking double their preference?
      ...
      ownershipDistribution.prefBInvestors =
        capTable[0].prefBInvestorsShares / numOfShares;
      ...
      if (prefBInvestorsTotalReturnCommon >= prefBInvestorsTotalReturn) {
        // Would B earn more converting to common shares than taking double their preference?
        ...
        ownershipDistribution.prefAInvestors =
          capTable[0].prefAInvestorsShares / numOfShares;

        // calculate pro rata amounts
        foundersProRata = remainder * ownershipDistribution.founders;
        prefAInvestorsProRata =
          remainder * ownershipDistribution.prefAInvestors;

        // recalculate total return
        let prefAInvestorsTotalReturnCommon = prefAInvestorsProRata;

        if (prefAInvestorsTotalReturnCommon >= prefAInvestorsTotalReturn) {
          // Would A earn more converting to common shares than taking double their preference?
      ...
      console.log(
        "C will stick with 2x their pref since it is more than converting to common and earning ",
        prefCInvestorsTotalReturnCommon
      );
      ...
      if (prefBInvestorsTotalReturnCommon >= prefBInvestorsTotalReturn) {
        ...
        console.log(
          "B will stick with 2x their pref since it is more than converting to common and earning ",
          prefBInvestorsTotalReturnCommon
        );
        ...
        numOfShares =
          numOfShares -
          capTable[0].prefBInvestorsShares -
          capTable[0].prefCInvestorsShares;
        ...
        if (prefAInvestorsTotalReturnCommon >= prefAInvestorsTotalReturn) {
          ...
          foundersTotalReturn =
            exitValue -
            prefAInvestorsTotalReturn -
            prefBInvestorsTotalReturn -
            prefCInvestorsTotalReturn;
        } else {
          console.log(
            "A will stick with 2x their pref since its more than converting to common and earning ",
            prefAInvestorsTotalReturnCommon
          );
          // calculate new amount of shares
          numOfShares = numOfShares - capTable[0].prefAInvestorsShares;

          // recalculate ownership distribution based off new amount of shares
          ownershipDistribution.founders =
            capTable[0].commonShares / numOfShares;
          ownershipDistribution.prefCInvestors =
            capTable[0].prefCInvestorsShares / numOfShares;
          ...
          foundersProRata = remainder * ownershipDistribution.founders;
          prefCInvestorsProRata =
            remainder * ownershipDistribution.prefCInvestors;

          // recalculate total return for remaining groups
          foundersTotalReturn = foundersLiqPref + foundersProRata;
          prefCInvestorsTotalReturn =
            prefCInvestorsLiqPref + prefCInvestorsProRata;
        }
      }
    }
  } else if (prefBInvestorsTotalReturn >= prefBInvestorsLiqPref * 2) {
    // Has Series B reached cap? If so, so has A.
    ...
    if (prefBInvestorsTotalReturnCommon >= prefBInvestorsTotalReturn) {
      ...
      if (prefAInvestorsTotalReturnCommon >= prefAInvestorsTotalReturn) {
        ...
        console.log(
          "A will stick with 2x their pref since it is more than converting to common and earning ",
          prefAInvestorsTotalReturnCommon
        );
        // calculate new amount of shares
        numOfShares = numOfShares - capTable[0].prefAInvestorsShares;
  ...
  } else if (prefAInvestorsTotalReturn >= prefAInvestorsLiqPref * 2) {
    // Has Series A reached cap?
